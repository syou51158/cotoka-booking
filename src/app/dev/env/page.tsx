  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-8">
      ...
      <p className="text-sm text-gray-600 mb-6">
        以下の必須設定を確認してください。未設定の場合は、プロジェクト直下の <code>.env.local</code> を作成し、
        <code>.env.example</code> を参照して値を設定してください（Next.js は再起動が必要）。
      </p>
      {missing.length > 0 ? (
        ...
        <div className="rounded border border-green-200 bg-green-50 p-3 mb-6">
          <div className="font-medium text-green-700">All required keys are set.</div>
        </div>
      )}
          ...
          {rows.map((r) => (
            <tr key={String(r.label)} className="border-b border-gray-200">
              ...
              <td className="py-2 font-mono text-gray-900">
                {typeof r.value === "boolean" ? (r.value ? "true" : "false") : r.value}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className="mt-6 text-sm text-gray-700">
        ...
        <ol className="list-decimal list-inside">
          ...
          <li><code>Project URL</code> を <code>SUPABASE_URL</code> に設定</li>
          <li><code>anon public</code> を <code>SUPABASE_ANON_KEY</code> に設定</li>
          <li><code>service_role</code> を <code>SUPABASE_SERVICE_ROLE_KEY</code> に設定</li>
